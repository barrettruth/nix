...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    system.activationScripts.barrettUiConfig.text = ''
      ...
      if [ -f "${XDG_CONFIG_HOME}/hypr/hyprland.conf" ] &&
        ${pkgs.gnugrep}/bin/grep -qx 'autogenerated = 1' "${XDG_CONFIG_HOME}/hypr/hyprland.conf"; then
        ${runAsUser} ${pkgs.coreutils}/bin/rm -f "${XDG_CONFIG_HOME}/hypr/hyprland.conf"
      fi
