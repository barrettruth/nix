local server = require('mx.server')
local view = require('mx.view')

local M = {}

local TABLINE_EXPR = "%!v:lua.require'mx.line'.render()"
...
local function apply_visibility()
    local file = server.runtime_dir() .. '/mx-bar'
...
function M.setup()
    apply_visibility()
    local group = vim.api.nvim_create_augroup('mx-line', { clear = true })
