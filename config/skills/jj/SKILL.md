---
name: jj
description: Use when working inside a jujutsu (jj) repository, or when Barrett asks about jj — the model, a command, a revset, or output that looks surprising. Reflects current jj behaviour, which has moved on from most training data. Establish real state with read-only commands, then report concisely.
---

# jj

Barrett's repositories are jujutsu on top of git. jj releases monthly and
...
`.jj/`. Barrett's user config is generated by nix from the `jjConf` block in
`~/.config/nix/modules/nixos/barrett/workstation.nix` and symlinked into place,
