...
Open an agent's changes for review in mux — the `vcs` window gets a unified
`:Diff review`, the `edit` window gets a quickfix of the changed files, in the
session you're attached to. Review-UI setup only: never edit, stage, commit,
push, mutate remotes, or run builds/tests.
...
```sh
python3 /home/barrett/.config/nix/config/skills/nvim-changes/scripts/review-ui.py [--repo <repo-root>] [<branch|worktree-path>]
```
