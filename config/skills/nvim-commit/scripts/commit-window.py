...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--dry-run",
        action="store_true",
        help="print the plan; do not stage or touch the nvim server",
    )
...
def main(argv: list[str]) -> int:
    ...
    if not run_lua(socket, STATUS_LUA):
        die("failed to open the fugitive commit buffer")
    ...
    print(f'commit-window: nvim server ready — {note}; drafted "{subject}"')
    return 0
